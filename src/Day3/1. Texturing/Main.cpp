...
HRESULT Initialize (HWND hWnd, UINT width, UINT height)
{
	...
	if (FAILED (ReadAndCompile (TEXT ("Day3\\1\\TextureVertexShader.hlsl"), RAC_VERTEXSHADER, "main", vertexShaderData, 4096, &vertexShaderDataLength)))
		return E_FAIL;
	if (FAILED (ReadAndCompile (TEXT ("Day3\\1\\TexturePixelShader.hlsl"), RAC_PIXELSHADER, "main", pixelShaderData, 4096, &pixelShaderDataLength)))
		return E_FAIL;
	...
	if (FAILED (LoadTexture2D (d3dDevice, TEXT ("Day3\\1\\Sample.png"), &texture)))
		return E_FAIL;
	if (FAILED (d3dDevice->CreateShaderResourceView (texture, nullptr, &shaderResourceView)))
		return E_FAIL;
