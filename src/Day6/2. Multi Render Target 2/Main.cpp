...
UINT vertices;
ID3D11Buffer* transformConstantBuffer, * drawConstantBuffer;
ID3D11InputLayout* inputLayout;
ID3D11VertexShader* vertexShader1, * vertexShader2;
ID3D11PixelShader* pixelShader1, * pixelShader2, * pixelShader3;
...
HRESULT Initialize (HWND hWnd, UINT width, UINT height)
{
	...
	if (FAILED (CreateModelFromOBJFile (d3dDevice, TEXT ("Day6\\2\\Sample.obj"), &vertexBuffer, &vertices)))
		return E_FAIL;
	...
	if (FAILED (d3dDevice->CreateBuffer (&constantBufferDesc, nullptr, &drawConstantBuffer)))
		return E_FAIL;

	uint8_t vertexShaderData[4096], pixelShaderData[4096];
	UINT vertexShaderDataLength, pixelShaderDataLength;
	if (FAILED (ReadAndCompile (TEXT ("Day6\\2\\DeferredVertexShader.hlsl"), RAC_VERTEXSHADER, "main", vertexShaderData, 4096, &vertexShaderDataLength)))
		return E_FAIL;
	if (FAILED (ReadAndCompile (TEXT ("Day6\\2\\DeferredPixelShader.hlsl"), RAC_PIXELSHADER, "main", pixelShaderData, 4096, &pixelShaderDataLength)))
		return E_FAIL;
	...
	if (FAILED (ReadAndCompile (TEXT ("Day6\\2\\DrawGBufferVertexShader.hlsl"), RAC_VERTEXSHADER, "main", vertexShaderData, 4096, &vertexShaderDataLength)))
		return E_FAIL;
	if (FAILED (ReadAndCompile (TEXT ("Day6\\2\\DrawGBufferPixelShader.hlsl"), RAC_PIXELSHADER, "main", pixelShaderData, 4096, &pixelShaderDataLength)))
		return E_FAIL;
	...
	if (FAILED (ReadAndCompile (TEXT ("Day6\\2\\DrawDepthBufferPixelShader.hlsl"), RAC_PIXELSHADER, "main", pixelShaderData, 4096, &pixelShaderDataLength)))
		return E_FAIL;
	if (FAILED (d3dDevice->CreatePixelShader (pixelShaderData, pixelShaderDataLength, nullptr, &pixelShader3)))
		return E_FAIL;
...
void Render (float dt)
{
	...
	immediateContext->VSSetConstantBuffers (0, 1, &drawConstantBuffer);
	immediateContext->PSSetShader (pixelShader2, nullptr, 0);

	DirectX::XMMATRIX translate = DirectX::XMMatrixTranslation (-0.5f, 0.5f, 0);
	immediateContext->UpdateSubresource (drawConstantBuffer, 0, nullptr, &translate, sizeof (DirectX::XMMATRIX), 0);
	immediateContext->PSSetShaderResources (0, 1, &renderTargetBufferSRVs[0]);
	immediateContext->Draw (4, 0);

	translate = DirectX::XMMatrixTranslation (0.5f, 0.5f, 0);
	immediateContext->UpdateSubresource (drawConstantBuffer, 0, nullptr, &translate, sizeof (DirectX::XMMATRIX), 0);
	immediateContext->PSSetShaderResources (0, 1, &renderTargetBufferSRVs[1]);
	immediateContext->Draw (4, 0);

	translate = DirectX::XMMatrixTranslation (-0.5f, -0.5f, 0);
	immediateContext->UpdateSubresource (drawConstantBuffer, 0, nullptr, &translate, sizeof (DirectX::XMMATRIX), 0);
	immediateContext->PSSetShaderResources (0, 1, &renderTargetBufferSRVs[2]);
	immediateContext->Draw (4, 0);

	translate = DirectX::XMMatrixTranslation (0.5f, -0.5f, 0);
	immediateContext->UpdateSubresource (drawConstantBuffer, 0, nullptr, &translate, sizeof (DirectX::XMMATRIX), 0);
	immediateContext->PSSetShader (pixelShader3, nullptr, 0);
	immediateContext->PSSetShaderResources (0, 1, &depthStencilBufferSRV);
	immediateContext->PSSetConstantBuffers (0, 1, &transformConstantBuffer);
	immediateContext->Draw (4, 0);
	ID3D11ShaderResourceView* const tSRV[1] = { NULL };
	immediateContext->PSSetShaderResources (0, 1, tSRV);
